...
For ship and scout tasks, the script asserts the resolved worktree is a genuine isolated worktree of the project's own repository - not firstmate's repo, its home, or another clone - and distinct from the primary checkout, aborting the spawn otherwise to prevent the worktree tangle of section 8.
The same identity test runs as a pre-flight on the project itself: a project directory that is not a git working-tree root (an interrupted clone, a directory that was never a repository) has no repository identity for a worktree to belong to, so the spawn is refused before the task window is created - repair or re-clone that project rather than retrying the spawn.
