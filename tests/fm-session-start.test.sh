...
EOF
  ...
  # Un-shim a tool that can only ever come from the fakebin: node ships in
  # /usr/bin on some distros, so removing it would not read as missing there.
  rm -f "$fakebin/gh-axi"
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # Un-shim a tool that can only ever come from the fakebin (see the ordering
  # test): node ships in /usr/bin on some distros and would not read as missing.
  rm -f "$fakebin/gh-axi"
