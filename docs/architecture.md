...
An exception: a turn-ended signal whose corresponding `.status` file on disk carries an unsurfaced captain-relevant line is always actionable, even when the crew is provably working, with `.hb-surfaced-*` dedup preventing re-churn.
...
That identity is anchored at the working-tree root on both sides of the comparison, because `git rev-parse` walks UP the tree and would otherwise hand a directory the identity of whatever repository encloses it - and projects live at `$FM_HOME/projects/<name>`, inside firstmate's own repo by construction.
So a project directory that is not itself a git working-tree root (an interrupted clone, a hand-made directory) has no repository identity for a worktree to match, and the spawn is refused up front, before the backend creates the task window; a candidate pane path merely INSIDE the clone or one of its worktrees is likewise not accepted as the task's worktree.
The worktree-discovery poll applies that same anchored identity test before accepting a candidate path, so a transient wrong read is rejected and the loop keeps polling until it sees a genuine worktree of the project or its budget runs out (60s by default, `FM_SPAWN_WORKTREE_TIMEOUT`), with the timeout error naming the last candidate path and why it was rejected.
The identity check in `validate_spawn_worktree` is then the last backstop when a worktree-discovery poll reads the wrong endpoint's cwd - either from a display-message target that silently fell back to the active window, or from a freshly created window that briefly reports the tmux server's cwd before its shell settles into the `-c` directory (see [`docs/tmux-backend.md`](tmux-backend.md), "Worktree-path discovery: two races, multiple defenses").
